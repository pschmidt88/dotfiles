...
# Configuration
FULL_WINDOW_GAP=20                                # Gap when no centered window gap is applied
WINDOW_PERCENTAGE_DEFAULT=75                      # Default percentage for automatic mode
WINDOW_PERCENTAGE_STEPS=(33 50 75)                # Percentages to cycle through in cycle mode
MONITOR_PATTERN="G34"                       		  # Monitor pattern to match in AeroSpace config (matches monitor name pattern)
...
# Returns 0 if the focused monitor is ultrawide, 1 otherwise
is_focused_ultrawide() {
	local monitor_name resolution width height aspect_ratio ultrawide
	monitor_name=$(aerospace list-monitors --focused | awk -F ' \\| ' '{print $2}')
	resolution=$(system_profiler SPDisplaysDataType | grep -A1 "$monitor_name" | grep "Resolution:" | head -1)
	...
	if [[ -n "$width" && -n "$height" && $height -gt 0 ]]; then
		...
		ultrawide=$(echo "$aspect_ratio >= $ULTRAWIDE_ASPECT_RATIO" | bc)
		[[ $ultrawide -eq 1 ]]
	else
		return 1
	fi
...
calculate_gap() {
	...
	if [[ -n "$width" && -n "$height" && $height -gt 0 ]]; then
		...
		if [[ $is_ultrawide -eq 1 ]]; then
			...
			echo "[{ monitor.\"$MONITOR_PATTERN\" = $gap_value }, $FULL_WINDOW_GAP]"
			return 0
		fi
	fi
...
# Early exit: focused monitor is not ultrawide — leave config untouched.
# The array format already provides the correct fallback value for non-ultrawide monitors.
if ! is_focused_ultrawide; then
	exit 0
fi
...
if [[ $window_count -ne 1 ]]; then
	...
	sed -i '' "s/^[[:space:]]*outer\.left = .*/    outer.left =       $size/; s/^[[:space:]]*outer\.right = .*/    outer.right =      $size/" "$config_path"
